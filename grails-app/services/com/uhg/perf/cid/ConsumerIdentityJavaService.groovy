...
import com.unitedhealthgroup.clinical.schema.contract.identity.v1.GetConsumerIdentityResponse
...
class ConsumerIdentityJavaService extends AbstractPerformanceAnalysisJob {
    ...
    def invokeService(String workers) {
        println "spawing java test with ${workers} workers"
        def threads = Integer.parseInt(workers)
        threads.times {
            runAsync {
                while(redisService.get(PerformanceConstants.CONSUMER_IDENTITY_JAVA_JOB) == PerformanceConstants.RUNNING) {
                    performTest()
                }
            }
        }
    ...
    private performTest() {
        ...
        def duration = benchmark {
            try {
                response = consumerIdentityJava.getConsumerIdentity(request)
        ...
        resultsService.saveResults(
                details?.error ? PerformanceConstants.CONSUMER_IDENTITY_JAVA_JOB_ERROR : PerformanceConstants.CONSUMER_IDENTITY_JAVA_JOB_RESULTS,
                new Result(testName: PerformanceConstants.CONSUMER_IDENTITY_JAVA_JOB, details: details?.msg ?: "", executionTime: duration))
    }
