...
<g:each in="${jobs}" var="job">
    <div class="row">
        <div class="job jobrunner col-xs-12" id= ${job.name}>
            <span class="cell"><b>${job.description}</b></span>
            <span class="cell">
                Workers: <g:select class="input-sm" name="${job.name}Workers" from="${1..job.maxWorkers}"/>
            </span>
            <span class="cell">
                <span class="buttons">
                    <input class="btn btn-sm btn-primary save" type="button" id="${job.name}Start" value="Start"
                           title="Start"/>
                    <input class="btn btn-sm btn-primary delete" type="button" id="${job.name}Stop" value="Stop"
                           title="Stop"/>
                </span>
            </span>
            <span class="cell" id="${job.name}Status"></span><img id="${job.name}Ajax"
                                                                  src="${resource(dir: 'images', file: 'spinner.gif')}">
            <span class="clear"></span>

            <div class="results">
                <span class="cell"><g:link action="resultDetails" controller="results"
                                           params="[queue: job.name + 'Results']"><b>Successful</b></g:link></span>
            ...
            <div class="results">
                <span class="cell"><g:link action="resultDetails" controller="results"
                                           params="[queue: job.name + 'Error']"><b>Errors</b></g:link></span>
...
<script type="text/javascript">
    ...
    (function ($) {
        $.widget('ace.jobrunner', {
                    ...
                    _initButtons: function (self) {
                        $("#" + self.id + "Start").click(function () {
                            self._startWorkers(self);
                        });
                        $("#" + self.id + "Stop").click(function () {
    ...
    $(function () {
        ...
        $jobrunner.jobrunner("setInterval", interval);
    });
